...
import type { IDBSnapshot } from "@/lib/idb";
...
function Sparkline({ data }: { data: IDBSnapshot[] }) {
...
export function DynamicsSection({ entryId, initialIntensity = 5, initialPainType = "aching" }: Props) {
  ...
  const { data: snapshots = [] } = useQuery<IDBSnapshot[]>({
    queryKey: ["snapshots", entryId],
    queryFn: () => idb.getSnapshots(entryId),
  });

  const createMutation = useMutation({
    mutationFn: () => idb.createSnapshot({ entryId, intensity, painType, note, date: date.toISOString() }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["snapshots", entryId] });
      setShowForm(false); setNote(""); setDate(new Date());
    },
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => idb.deleteSnapshot(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["snapshots", entryId] }),
  });
    ...
    <div className="flex flex-col gap-3">
      {snapshots.length > 0 && (
        <div className="flex items-center gap-3 bg-muted/20 rounded-2xl border border-border/60 p-4">
          ...
          <div className="flex flex-col items-end text-xs text-muted-foreground">
            ...
            {snapshots.length >= 2 && <span>Нач.: {snapshots[0].intensity}</span>}
          </div>
        </div>
      )}
      ...
      {snapshots.slice().reverse().map((s) => (
        <div key={s.id} className="bg-card rounded-2xl border border-border/60 p-3 flex items-start gap-3">
          ...
          <button onClick={() => deleteMutation.mutate(s.id!)} className="w-7 h-7 rounded-lg flex items-center justify-center text-muted-foreground hover:text-destructive transition-colors">
      ...
      {showForm ? (
        <div className="bg-muted/20 rounded-2xl border border-border/60 p-4 flex flex-col gap-4">
          ...
          <div>
            ...
            <input type="range" min={1} max={10} step={1} value={intensity} onChange={(e) => setIntensity(Number(e.target.value))} className="w-full accent-primary" />
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {PAIN_TYPES.map(({ value, label }) => (
                <button key={value} onClick={() => setPainType(value)}
          ...
          <textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="Заметка (необязательно)..." rows={2}
        ...
        <button onClick={() => { setIntensity(initialIntensity); setPainType(initialPainType); setShowForm(true); }}
