...
import { Plus, Trash2, Pill, Dumbbell, Stethoscope, MoreHorizontal, ThumbsUp, Minus, ThumbsDown, ChevronDown, ChevronUp } from "lucide-react";
  ...
  const createMutation = useMutation({
    mutationFn: () =>
      fetch(`/api/entries/${entryId}/treatments`, {
        ...
        body: JSON.stringify({ type, title: title.trim() || TYPE_LABELS[type], notes, result, date: new Date(date).toISOString() }),
      }).then((r) => r.json()),
    onSuccess: () => {
