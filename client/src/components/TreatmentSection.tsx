...
import type { IDBTreatment } from "@/lib/idb";
...
const TYPE_ICONS: Record<string, React.ElementType> = { medication: Pill, procedure: Stethoscope, exercise: Dumbbell, other: MoreHorizontal };
const TYPE_LABELS: Record<string, string> = { medication: "Лекарство", procedure: "Процедура", exercise: "Упражнение", other: "Другое" };

export function TreatmentSection({ entryId }: { entryId: number }) {
  ...
  const { data: treatments = [] } = useQuery<IDBTreatment[]>({
    queryKey: ["treatments", entryId],
    queryFn: () => idb.getTreatments(entryId),
  });

  const createMutation = useMutation({
    mutationFn: () => idb.createTreatment({
      entryId, type,
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["treatments", entryId] });
      setShowForm(false); setTitle(""); setNotes(""); setResult(""); setType("medication"); setDate(new Date());
    },
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => idb.deleteTreatment(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["treatments", entryId] }),
  });
  ...
  return (
    <div className="flex flex-col gap-3">
      {treatments.map((t) => {
        ...
        const resultInfo = RESULT_OPTIONS.find(o => o.value === t.result);
        ...
        return (
          <div key={t.id} className="bg-card rounded-2xl border border-border/60 p-4 flex flex-col gap-2">
            <div className="flex items-start gap-3">
              ...
              <button onClick={() => deleteMutation.mutate(t.id!)} className="w-7 h-7 rounded-lg flex items-center justify-center text-muted-foreground hover:text-destructive transition-colors">
      ...
      {showForm ? (
        <div className="bg-muted/20 rounded-2xl border border-border/60 p-4 flex flex-col gap-3">
          ...
          <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={`Название (${TYPE_LABELS[type].toLowerCase()})`} className="rounded-xl text-sm" />
          <DateTimePicker value={date} onChange={setDate} label="Дата и время" />
          <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Дозировка, описание процедуры..." rows={2}
          ...
          <div>
            ...
            <div className="flex gap-2">
              {RESULT_OPTIONS.map(({ value, label, icon: Icon, color }) => (
                <button key={value} onClick={() => setResult(result === value ? "" : value)}
                  className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl border text-xs font-medium transition-colors ${result === value ? color : "bg-card border-border/60 text-muted-foreground"}`}>
